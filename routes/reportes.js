...
router.get('/asistencia', verifyToken, requirePermiso('ver_reportes'), (req, res) => {
  ...
  const params  = [fecha_inicio, fecha_fin, req.user.institucion_id];
  const filtroE = curso_id ? 'AND e.curso_id = ?' : '';
  if (curso_id) params.push(Number(curso_id));

  const estudiantes = db.prepare(`
    SELECT e.id, e.nombre, e.apellido, e.dni, e.curso_id,
  ...
  // Días únicos con asistencia registrada en el período
  const diasP  = [req.user.institucion_id, fecha_inicio, fecha_fin];
  const diasF  = curso_id ? 'AND curso_id=?' : '';
  if (curso_id) diasP.push(Number(curso_id));
  const { dias_registrados } = db.prepare(`
    SELECT COUNT(DISTINCT fecha) AS dias_registrados
    FROM   asistencias
    WHERE  institucion_id=? AND tipo_asistencia='general' AND fecha BETWEEN ? AND ? ${diasF}
  `).get(...diasP);

  res.json({
    estudiantes: estudiantes.map(e => ({
      ...e,
      porcentaje: e.total_registros > 0
        ? Math.round((e.presencias / e.total_registros) * 100)
        : null
    })),
    dias_registrados
  });
