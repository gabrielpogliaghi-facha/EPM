...
// ── BULK AUTORIZAR (debe ir ANTES de PUT /:id para no colisionar) ─────────────
// PUT /api/estudiantes/bulk-autorizar
// Solo marca (pone en 1); nunca desmarca.
router.put('/bulk-autorizar', verifyToken, requirePermiso('editar_estudiantes'), (req, res) => {
  ...
  const sql = `UPDATE estudiantes SET ${setParts.join(', ')}, updated_at=datetime('now')
               WHERE id IN (${ph}) AND institucion_id = ? AND activo = 1`;
  const r = db.prepare(sql).run(...ids.map(Number), req.user.institucion_id);
  res.json({ success: true, actualizados: r.changes });
});

// ── FOTO: subir ──────────────────────────────────────────────────────────────
router.post('/:id/foto', verifyToken, requirePermiso('editar_estudiantes'), upload.single('foto'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'No se recibió imagen' });
  const est = db.prepare('SELECT foto_path FROM estudiantes WHERE id=? AND institucion_id=? AND activo=1')
    .get(Number(req.params.id), req.user.institucion_id);
  if (!est) { fs.unlink(req.file.path, () => {}); return res.status(404).json({ error: 'Estudiante no encontrado' }); }

  // Borra foto anterior si existía
  if (est.foto_path) {
    const old = path.join(__dirname, '..', 'uploads', 'estudiantes', path.basename(est.foto_path));
    fs.unlink(old, () => {});
  }
  const fotoPath = `/uploads/estudiantes/${req.file.filename}`;
  db.prepare("UPDATE estudiantes SET foto_path=?, updated_at=datetime('now') WHERE id=?")
    .run(fotoPath, Number(req.params.id));
  res.json({ foto_path: fotoPath });
});

// ── FOTO: quitar ─────────────────────────────────────────────────────────────
router.delete('/:id/foto', verifyToken, requirePermiso('editar_estudiantes'), (req, res) => {
  const est = db.prepare('SELECT foto_path FROM estudiantes WHERE id=? AND institucion_id=? AND activo=1')
    .get(Number(req.params.id), req.user.institucion_id);
  if (!est) return res.status(404).json({ error: 'Estudiante no encontrado' });
  if (est.foto_path) {
    const p = path.join(__dirname, '..', 'uploads', 'estudiantes', path.basename(est.foto_path));
    fs.unlink(p, () => {});
    db.prepare("UPDATE estudiantes SET foto_path=NULL, updated_at=datetime('now') WHERE id=?")
      .run(Number(req.params.id));
  }
  res.json({ success: true });
});

// POST /api/estudiantes/importar — importación masiva desde CSV (pre-validado en frontend)
router.post('/importar', verifyToken, requirePermiso('crear_estudiantes'), (req, res) => {
...
// GET /api/estudiantes?curso_id=&buscar=
router.get('/', verifyToken, requirePermiso('ver_estudiantes'), (req, res) => {
  ...
  if (curso_id) { sql += ' AND e.curso_id = ?'; params.push(Number(curso_id)); }
  if (buscar) {
    sql += ' AND (e.nombre LIKE ? OR e.apellido LIKE ? OR e.dni LIKE ?)';
    const t = `%${buscar}%`; params.push(t, t, t);
  }
...
router.get('/:id', verifyToken, requirePermiso('ver_estudiantes'), (req, res) => {
  const e = db.prepare(`
    SELECT e.*, c.nombre AS curso_nombre
    FROM   estudiantes e
    LEFT JOIN cursos c ON e.curso_id = c.id
    WHERE  e.id = ? AND e.institucion_id = ? AND e.activo = 1
  `).get(Number(req.params.id), req.user.institucion_id);
...
router.post('/', verifyToken, requirePermiso('crear_estudiantes'), (req, res) => {
...
router.put('/:id', verifyToken, requirePermiso('editar_estudiantes'), (req, res) => {
  const { nombre, apellido, dni, cuit, fecha_nacimiento,
          tutor_nombre, tutor_dni, direccion, curso_id,
          auth_imagen, auth_general, auth_boleto } = req.body;
  if (!nombre?.trim() || !apellido?.trim() || !dni?.trim())
  ...
  const est = db.prepare(
    'SELECT id FROM estudiantes WHERE id = ? AND institucion_id = ? AND activo = 1'
  ).get(Number(req.params.id), req.user.institucion_id);
  if (!est) return res.status(404).json({ error: 'Estudiante no encontrado' });
  try {
    db.prepare(`
...
// DELETE /api/estudiantes/:id — soft delete (activo=0), preserva todos los registros
router.delete('/:id', verifyToken, requirePermiso('editar_estudiantes'), (req, res) => {
  const est = db.prepare(
    'SELECT id, nombre, apellido FROM estudiantes WHERE id = ? AND institucion_id = ? AND activo = 1'
  ).get(Number(req.params.id), req.user.institucion_id);
  if (!est) return res.status(404).json({ error: 'Estudiante no encontrado' });
  db.prepare("UPDATE estudiantes SET activo=0, updated_at=datetime('now') WHERE id=?").run(Number(req.params.id));
  res.json({ success: true, nombre: `${est.apellido}, ${est.nombre}` });
});
