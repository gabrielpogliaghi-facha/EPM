...
**Sistema completo sin dependencia de email. Invitaciones por link. Equipo docente unificado dentro de Usuarios. Reuniones institucionales con acta tipo WhatsApp y @menciones con notificaciones. Finanzas en estructura. Navegación con historial (hash routing) para mobile. Reporte por nivel/curso con completitud de datos. Inscripciones multi-nivel (mismo instrumento en niveles distintos). Autoservicio de perfil personal ("Mi perfil") con cambio de contraseña propio. Cumpleaños de estudiantes Y equipo (cualquier rol) funcionando correctamente en Dashboard y Calendario.**
